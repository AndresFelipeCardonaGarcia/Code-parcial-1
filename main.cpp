...
void elegir_matriz(int hora_inicio,int hora_fin,char dia,int L[],int M[],int W[],int J[],int V[],int S[],int D[],int num){
    switch(dia) {
      case 'L':
      case 'l':
        tiempo_libre(hora_inicio,hora_fin,L);
        reemplazar(hora_inicio, hora_fin, L,num);
        break;
      case 'M':
      case 'm':
        tiempo_libre(hora_inicio,hora_fin,M);
        reemplazar(hora_inicio, hora_fin, M,num);
        break;
      case 'W':
      case 'w':
        tiempo_libre(hora_inicio,hora_fin,W);
        reemplazar(hora_inicio, hora_fin, W,num);
        break;
      case 'J':
      case 'j':
        tiempo_libre(hora_inicio,hora_fin,J);
        reemplazar(hora_inicio, hora_fin, J,num);
        break;
      case 'V':
      case 'v':
        tiempo_libre(hora_inicio,hora_fin,V);
        reemplazar(hora_inicio, hora_fin, V,num);
        break;
      case 'S':
      case 's':
        tiempo_libre(hora_inicio,hora_fin,S);
        reemplazar(hora_inicio, hora_fin, S,num);
        break;
      case 'D':
      case 'd':
        tiempo_libre(hora_inicio,hora_fin,D);
...
int main()
{
    ...
    cout << "Para usar Horario predenterminado (9pm:7am) presione s\nPara elegir un horario personalizado presione n\n";
    cin >> true_false;
    num=1;
    switch (true_false) {
        case 's':
        ...
        case 'N':
            ...
            break;
        default:
            cout << "Opcion invalida. Para usar Horario predenterminado (10pm:7am) presione s\nPara elegir un horario personalizado presione n\n" << endl;cin>>true_false;
    //break;
    }
    cout << "\033c";
    cout<<"Tiene alguna actividad programada en la cual no puede estudiar?"<<endl;
    cout<<"Si ese es el caso presiona s, sino presiona cualquier tecla "<<endl;cin >> true_false;
    if (true_false == 's'){
        num=2;
            while (true){
            cout<<"Elija dia que tiene ocupado, teniendo encuenta lo siguiente\n L es lunes, M es martes W es miercoles J es jueves V es viernes, S es sabado y D es domingo"<<endl;cin>>dia;
            ...
            cout<<"Si no tiene mas actividades fuera de la universidad presiona n, en caso contrario presione cualquier tecla"<<endl;cin>>true_false;
...
break;
    ...
    while (true){
        ...
        while(true){
            cout<<"Elija dia que tiene su clase,teniendo encuenta lo siguiente\n L es lunes, M es martes W es miercoles J es jueves V es viernes, S es sabado y D es domingo"<<endl;cin>>dia;
